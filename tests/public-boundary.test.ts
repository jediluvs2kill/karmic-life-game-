...
test('public runtime uses an isolated origin, storage key, and explicit publication notices',async()=>{
 ...
 for(const file of ['vite.config.ts','server/world-api.mjs','server/swarm.mjs','scripts/launch.mjs']){const text=await read(file);assert.ok(text.includes('5174'));assert.ok(!text.includes('5173'));}
