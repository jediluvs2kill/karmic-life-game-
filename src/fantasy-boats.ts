...
export type BoatKind='wind-cutter'|'lotus-skiff'|'turtle-cruiser'|'crystal-catamaran'|'skyfish-sloop'|'rune-barge';
export type Fleet='azure'|'coral';
export type BoatPlan={id:string;name:string;kind:BoatKind;fleet:Fleet;phase:number;direction:1|-1;speed:number;lane:number;color:string;accent:string};
export type BoatPose={x:number;y:number;z:number;yaw:number;bob:number};

/** Six peaceful fantasy vessels stage theatrical magical skirmishes in open water. */
export const fantasyFleet:BoatPlan[]=[
 {id:'boat-nimbus',name:'Nimbus Wind Cutter',kind:'wind-cutter',fleet:'azure',phase:.05,direction:1,speed:.055,lane:-1,color:'#d8f6ff',accent:'#54c8eb'},
 {id:'boat-lotus',name:'Lotus Petal Skiff',kind:'lotus-skiff',fleet:'coral',phase:.21,direction:-1,speed:.047,lane:1,color:'#ffb5cf',accent:'#ffe295'},
 {id:'boat-turtle',name:'Jade Turtle Cruiser',kind:'turtle-cruiser',fleet:'azure',phase:.38,direction:1,speed:.038,lane:0,color:'#80d1a5',accent:'#f4d76f'},
 {id:'boat-crystal',name:'Crystal Twinrunner',kind:'crystal-catamaran',fleet:'coral',phase:.55,direction:-1,speed:.061,lane:-1,color:'#bdb1ff',accent:'#70e2ef'},
 {id:'boat-skyfish',name:'Skyfish Sloop',kind:'skyfish-sloop',fleet:'azure',phase:.72,direction:1,speed:.052,lane:1,color:'#8dc7ff',accent:'#fff0a1'},
 {id:'boat-rune',name:'Ember Rune Barge',kind:'rune-barge',fleet:'coral',phase:.88,direction:-1,speed:.034,lane:0,color:'#e88765',accent:'#ffd06d'}
];
...
export function sampleBoat(plan:BoatPlan,time:number,reducedMotion=false):BoatPose{
 ...
 const x=Math.cos(angle)*34,z=46+plan.lane*2.5+Math.sin(angle)*3.4,dx=-Math.sin(angle)*34*plan.direction,dz=Math.cos(angle)*3.4*plan.direction;
...
export function sampleFleetVolley(time:number,reducedMotion=false){
 ...
 return fantasyFleet.map((boat,index)=>{
  const target=fantasyFleet[(index+1)%fantasyFleet.length],from=sampleBoat(boat,time),to=sampleBoat(target,time);
...
export function createFantasyBoats(scene:THREE.Scene,options:{reducedMotion?:boolean}={}){
 ...
 const hulls=batch('unique fantasy hulls',new THREE.SphereGeometry(1,14,8),material,12),decks=batch('boat decks and turtle shells',new THREE.SphereGeometry(1,12,8),material,6);
 const masts=batch('boat masts',new THREE.CylinderGeometry(1,1,1,8),dark,6),sails=batch('boat sails',new THREE.PlaneGeometry(1,1),sailMaterial,6);
 const wings=batch('lotus petals and skyfish fins',new THREE.ConeGeometry(1,1,8),material,12),cannons=batch('magic pulse cannons',new THREE.CylinderGeometry(1,1,1,8),dark,12);
 const eyes=batch('anime prow eyes',new THREE.SphereGeometry(1,8,6),dark,12);
 ...
 function update(time:number,visible=true){root.visible=visible;if(!visible)return;fantasyFleet.forEach((boat,i)=>{const pose=sampleBoat(boat,time,options.reducedMotion),twin=boat.kind==='crystal-catamaran',wide=boat.kind==='lotus-skiff',armored=boat.kind==='turtle-cruiser',long=boat.kind==='rune-barge',fish=boat.kind==='skyfish-sloop';
  ...
  place(decks,i,pose,[0,.6,-.05],armored?[.72,.55,1]:long?[.7,.24,1.45]:[.62,.23,.9]);place(masts,i,pose,[0,1.2,-.12],[.07,1.35,.07]);place(sails,i,pose,[0,1.55,.02],[boat.kind==='wind-cutter'?1.5:1.1,boat.kind==='wind-cutter'?1.65:1.25,1],[0,0,0]);
 });
