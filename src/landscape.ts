...
export function buildLandscape(projects:LivingProject[],cube:Cube,decor:Decor){
 ...
 for(let x=-39;x<=40;x++)for(let z=-33;z<=39;z++){const edge=x*x/(37*37)+(z-3)*(z-3)/(34*34);if(edge<1+(noise(Math.floor(x/3),Math.floor(z/3))-.5)*.12)cells.add(key(x,z));}
 ...
 for(const k of cells){const [x,z]=k.split(',').map(Number),h=groundHeight(x,z),edge=[[1,0],[-1,0],[0,1],[0,-1]].some(([dx,dz])=>!cells.has(key(x+dx,z+dz))),r=noise(x,z),near=points.some(p=>Math.hypot(x-p.x,z-p.z)<(landmarks[p.id]?5.6:3.2));
  ...
  const building=projects.some(p=>{const pos=projectPosition(p),scale=landmarks[p.id]?.scale??1;return Math.abs(x-pos.x)<2.8*scale&&Math.abs(z-pos.z)<2.4*scale;});
